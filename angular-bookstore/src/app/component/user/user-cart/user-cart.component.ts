...
export class UserCartComponent implements OnInit {
  ...
  getTotal(quantity: number = 0, mrpPrice: number = 0): number {
   ...
   placeOrder(): void {
    ...
    const bookItems: Array<Book> = [];
    this.cartList.forEach((item: Cart) => {
      bookItems.push(item?.book);
  ...
  onIncreaseQunatity(cart: Cart): void {
    ...
    console.log( this.cartListBackup[indexBackup].quantity , '>>>>>>' , (cart.book?.quantity ))
    if (qty > (cart.book?.quantity  + this.cartListBackup[indexBackup].quantity) ) {
